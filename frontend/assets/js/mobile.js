...
document.addEventListener("DOMContentLoaded", () => {
  ...
  .then((html) => {
    // Fix paths for subpages (prefix with ../)
    const fixedHtml = html.replace(/(src|href)="(?!(?:https?:\/\/|\/|..\/))([^"]+)"/g, '$1="../$2"');
    document.getElementById("course-overview-mobile-placeholder").innerHTML = fixedHtml;
     // --- MOBILE LOGIC ---
      document
...
document.addEventListener("DOMContentLoaded", () => {
  fetch("../sections/getting-started.html")
    ...
    .then((html) => {
      // Fix paths for subpages (prefix with ../)
      const fixedHtml = html.replace(/(src|href)="(?!(?:https?:\/\/|\/|..\/))([^"]+)"/g, '$1="../$2"');
      document.getElementById("getting-started-placeholder").innerHTML = fixedHtml;
    })
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("../sections/working-with-us.html")
    ...
    .then((html) => {
      // Fix paths for subpages (prefix with ../)
      const fixedHtml = html.replace(/(src|href)="(?!(?:https?:\/\/|\/|..\/))([^"]+)"/g, '$1="../$2"');
      document.getElementById("accessing-us-section-placeholder").innerHTML = fixedHtml;
    });
  // Sponsors Section
  fetch("../sections/sponsors.html")
    ...
    .then((html) => {
      // Fix paths for subpages (prefix with ../)
      const fixedHtml = html.replace(/(src|href)="(?!(?:https?:\/\/|\/|..\/))([^"]+)"/g, '$1="../$2"');
      document.getElementById("sponsors-section-placeholder").innerHTML = fixedHtml;
    })
