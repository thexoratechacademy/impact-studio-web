...
document.addEventListener("DOMContentLoaded", () => {
  fetch("../sections/mission-section.html")
    ...
    .then((html) => {
      // Fix paths for subpages (prefix with ../)
      const fixedHtml = html.replace(/(src|href)="(?!(?:https?:\/\/|\/|..\/))([^"]+)"/g, '$1="../$2"');
      document.getElementById("mission-section").innerHTML = fixedHtml;
    })
...
document.addEventListener("DOMContentLoaded", () => {
  fetch("../sections/Core-Val-Section.html")
    ...
    .then((html) => {
      // Fix paths for subpages (prefix with ../)
      const fixedHtml = html.replace(/(src|href)="(?!(?:https?:\/\/|\/|..\/))([^"]+)"/g, '$1="../$2"');
      document.getElementById("core-values-section").innerHTML = fixedHtml;
    })
    .catch((err) => console.error("Failed to load footer:", err));
});
...
document.addEventListener("DOMContentLoaded", () => {
  fetch("../sections/Team-section.html")
    ...
    .then((html) => {
      // Fix paths for subpages (prefix with ../)
      const fixedHtml = html.replace(/(src|href)="(?!(?:https?:\/\/|\/|..\/))([^"]+)"/g, '$1="../$2"');
      document.getElementById("Team-section").innerHTML = fixedHtml;
    })
    .catch((err) => console.error("Failed to load footer:", err));
});
