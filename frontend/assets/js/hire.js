document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    // Inject content (with path correction for subpages)
    const fixPaths = (html) => html.replace(/(src|href)="(?!(?:https?:\/\/|\/|..\/))([^"]+)"/g, '$1="../$2"');
    
    navbarPlaceholder.innerHTML = fixPaths(navbarHtml);
    hirePlaceholder.innerHTML = hireHtml;
    topPlaceholder.innerHTML = topHtml;
    footerPlaceholder.innerHTML = fixPaths(footerHtml);
